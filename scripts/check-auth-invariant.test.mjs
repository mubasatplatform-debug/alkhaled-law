...
test("the build side resolves a workspace's shipped app-env", () => {
  // Built against a workspace this test owns, not the surrounding checkout:
  // `.grok/app-env.json` is gitignored, so an app built on this template ships
  // none and the real root would resolve the auth-on default instead.
