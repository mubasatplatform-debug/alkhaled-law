...
/**
 * An empty workspace for the tests that assert the injector's OWN defaults.
 *
 * `normalizeHeadContext` resolves its share-card identity from `cwd` — both
 * `src/lib/og/site.json` and `public/og.jpg`. Left to default to `process.cwd()`
 * those tests read whatever app is built on this template: a configured
 * site.json renames og:title, and a real public/og.jpg upgrades the expected
 * og.grok.me placeholder to a custom card. Pointing them at a bare directory
 * keeps them testing the injector instead of the surrounding repository.
 */
