...
/**
 * The prompts pinned below live in `.grok/skills/` and `AGENTS.md`, both of
 * which are gitignored: they exist in the template workspace and not in a
 * checkout of an app built on it. With no prompt to read there is nothing for
 * these three tests to pin, so they skip instead of failing the app's CI.
 */
const MISSING_PROMPTS = [".grok/skills/og/SKILL.md", "AGENTS.md"].filter(
  (rel) => !existsSync(join(TEMPLATE_ROOT, rel)),
);
const SKIP_WITHOUT_PROMPTS =
  MISSING_PROMPTS.length > 0 && `not in this workspace: ${MISSING_PROMPTS.join(", ")}`;

test(
  "SKILL.md and AGENTS.md name the marker path and bound this script uses",
  { skip: SKIP_WITHOUT_PROMPTS },
  () => {
